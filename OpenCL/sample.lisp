...
(define (new-size_t) (raw type-vector-raw (repeat 0 (vm:wordsize))))
(define (new-int)    (raw type-vector-raw '(0 0 0 0)))
...
(define n (raw type-vector-raw '(0 0 0 0)))
(check
   (clGetPlatformIDs 0 null n))
(define n (->int n))
(print "Number of available platforms: " n)

(define platforms (raw type-vector-raw (repeat 0 (* n (vm:wordsize)))))
(check
...
(define (cl:GetPlatformInfo platform name)
   (let*((length (raw type-vector-raw '(0 0 0 0)))
         (_ (check (clGetPlatformInfo platform name 0 null length)))
         (length (->int length))
         (info (raw type-string (repeat 0 length)))
...
(define devices-count (raw type-vector-raw '(0 0 0 0)))
...
(define devices (raw type-vector-raw (repeat 0 (* devices-count (vm:wordsize)))))
(check
...
(define error (raw type-vector-raw '(0 0 0 0)))
...
;      (define log (raw type-string (repeat 0 sizeof-log)))
