      ...
      (define mcp-syscalls
         (tuple
            ...
            (λ (id a b c todo done state tc)
               ...
               (tc todo (cons (tuple id a) done) state))
            ...
            ;   (tc (cons (tuple id (λ () (cont 'already-profiling))) todo) done state))
            ;
            ...
            ;      (tc (cons (tuple id (λ () (cont prof))) todo) done
            ;         (del state 'prof))))
            ...
            (λ (id cont opts c todo done state tc)
               ...
               (lets
                  ...
                  (tc (cons (tuple id por-state) todo) done state)))
      ))
                  ...
                  (lets ((fini stp (step-parallel st)))
                     (cond
                        ((not fini)
                           ...
                           (thread-controller todo (cons (tuple id stp) done) state))
                        ((eq? fini #true)
