   ...
   (begin
      ...
      (define (backtrack l r p why)
         (if (null? l)
            ...
            (let ((hd (car l)))
               (if (eq? (type hd) type-enum+) ; TODO: change to integer?
      ...
      (define (drop l x)
         (cond
            ((eq? (car l) x)
               (cdr l))
            ((eq? (type (car l)) type-enum+) ; todo: change to (char?) or (rune?)
      ...
      (define (parse parser unused-path data fail-val) ; todo: use path
         (let loop ((try (λ () (parser '() data 0 parser-succ))))
            (let* ((l r p val (try)))
                (cond
                  ((not l)
                     fail-val)
