...
(begin
   ...
   (define system
      ...
      (case-lambda
         ...
         ((commands in)
            (system commands in #f #f (list (if in (car in)))))
         ((commands in out)
            (system commands in out #f (list (if in (car in)) (if out (cdr out)))))
         ((commands in out err)
            (system commands in out err (list (if in (car in)) (if out (cdr out)) (if err (cdr err)))))))
