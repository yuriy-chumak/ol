...
(define cls (cond
   (win32?
      ...
      (let*((kernel32 (dlopen "kernel32.dll"))
            ...
            (GetConsoleScreenBufferInfo (dlsym kernel32 type-vptr "GetConsoleScreenBufferInfo" type-vptr type-vector-raw))
            (FillConsoleOutputCharacter (dlsym kernel32 type-vptr "FillConsoleOutputCharacterW" type-vptr type-fix+ type-int+ type-int+ type-vector-raw))
            (FillConsoleOutputAttribute (dlsym kernel32 type-vptr "FillConsoleOutputAttribute" type-vptr type-int+ type-int+ type-int+ type-vector-raw))
      ...
      (lambda ()
         (let ((hConsole (GetStdHandle STD_OUTPUT_HANDLE))
               ...
               (cCharsWritten (vm:new-raw-object type-vector-raw 4))) ; DWORD
