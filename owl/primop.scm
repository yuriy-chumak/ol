...
(define-library (owl primop)
   ...
   (begin
      ...
      (define (primoP name code c)
         (let*
            ((arity     (car code))
             (bytecode  (cdr code))
             (len       (length bytecode))
             (primitive (raw
                          (append (list 25 arity 0 len)
                                  (append bytecode '(17))) ; 17 == ARITYERROR
                          type-bytecode #false)))
            (tuple name
                   (car bytecode)
                   (- arity 1)
      ...
      (define (primop name code c)
         (let*
            ((arity     (car code))
             ...
             (len       (length bytecode))
             (primitive (raw
                          (append (list 25 (+ arity 1) 0 len)
                                  (append bytecode '(17))) ; 17 == ARITYERROR
                          type-bytecode #false)))
            (tuple name (car bytecode) arity c primitive)))
...
;     (define set!        (func '(4 10 4 5 6 7    24 7)))
      ...
      (define primitives
         (list
            ...
            (primop 'sys-prim   '(4 . (63 4 5 6 7  8  24 8))  1)
            ...
            (primop 'cons       '(2 . (51 4 5      6  24 6))  1)
            
            (primop 'car        '(1 . (52 4        5  24 5))  1)
            (primop 'cdr        '(1 . (53 4        5  24 5))  1)
            (primop 'set-car!   '(2 . (11 4 5      6  24 6))  2) ; why 2 ?
            (primop 'set-cdr!   '(2 . (12 4 5      6  24 6))  2) ; why 2 ?
            
            (primop 'ref        '(2 . (47 4 5      6  24 6))  1) ; op47 = ref t o r = prim_ref(A0, A1)
...
;            (tuple 'ref          47 2 1 ref)   ;;
      ...
      (define (get-primitive name)
         (let loop ((p primops))
            (if (eq? (car (car p)) name)
                (car p)
                (loop (cdr p)))))
                
;      (define sys-prim (get-primitive 'sys-prim))
