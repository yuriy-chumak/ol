...
<pre><code data-language="scheme">(import (lib winapi))
   ...
   Мы можем сразу приступить к созданию простого окна (в отличие от Linux, нам не надо выбирать дисплей и рабочий стол, Windows все сделает вместо нас).
...
##### <a name="context-linux"></a>Linux
   ...
   Это довольно простая операция. Первым шагом создаем структуру с нужными параметрами, вторым создаем контекс.
...
##### <a name="context-win32"></a>Windows
   ...
   В Windows все несколько сложнее. Первым шагом нам надо создать контекст устройства для окна, потом выбрать для него (для контекста, не для окна) нужный нам набор параметров и уже тогда создавать контекст OpenGL.
...
##### <a name="loop-linux"></a>Linux
   ...
   Функция XPending сообщит нам, есть ли в очереди окна ожидающие на обработку сообщения. XNextEvent сложит это сообщение XEvent переменную. glXMakeCurrent активирует контекст OpenGL позволяя нам рисовать (в данном случае очистить окно с помощью заданного по умолчанию цвета).
...
#### <a name="all"></a>Объединение всего вышеперечисленного в один платформонезависимый код
   ...
   Код библиотеки несколько усложнен в сравнении с вышеприведенными врезками, но это усложнение не касается принципиальной асти а только импорта нужных функций в зависимости от окружения, в котором библиотека запускается.
