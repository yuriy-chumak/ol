...
<pre><code id="list4" data-language="scheme">(print "2+2 = " (+ 2 2))</code><button class="doit" onclick="doit(list4.textContent)">отправить в терминал</button></pre>
   ...
   Пустой список создается через (list ) и имеет специальное имя #null.
...
#### #true и #false
   ...
   В качестве первого непосредственного значения, представленного в Lisp, мы выберем булевыю пару констант true/false. В Scheme они обозначаются как #false (сокращенно #f) и #true (#t). Эти значения являются полностью самостоятельными константами и сравниваются только с собой. Таким образом правдой являются только два выражения: (eq? #t #t) и (eq? #f #f).
   
   Надо отметить, что оператор if работает со всеми значениями, не только #t и #f. Однако, как правду он трактует все, что не является #false - таким образом #f является сильным предикатом, в отличие от своей пары #t.
   
   Обратите внимание на такой момент, так как он довольно часто вызывает недопонимание - специальное значение #null (пустой список) не является #false. Таким образом (if #null 1 2) вернет 1.
