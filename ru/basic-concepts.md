...
Наименее существенная, но первая из описываемых конструкций языка - комментарий. Комментарий, это полностью игнорируемая транслятором часть текста программы, предназначенная для человека, не для компилятора.
...
Списки являются необходимейшей конструкцией Lisp - даже его название расшифровывается как "обработчик списков" (LISt Processor). И сама программа, которую дают выполнить Lisp'у - как ни странно, тоже список.
...
<pre><code id="list1" data-language="scheme"><button class="doit" onclick="doit(list1.textContent)">отправить в терминал</button>
(* 1 2 3 4 5 6 7 8 9)</code></pre>

<pre><code id="list2" data-language="scheme"><button class="doit" onclick="doit(list2.textContent)">отправить в терминал</button>
(print "Hello John!")</code></pre>

<pre><code id="list3" data-language="scheme"><button class="doit" onclick="doit(list3.textContent)">отправить в терминал</button>
(print "Ну или привет, если так понятнее.")</code></pre>

<pre><code id="list4" data-language="scheme"><button class="doit" onclick="doit(list4.textContent)">отправить в терминал</button>
...
Пустой список создается через (list первый-элемент второй-элемент .. Nй-элемент) и имеет, ввиду своего широкого использования, сразу несколько обозначений: #null, null и '(). Какое из них использовать - дело вкуса и интуиции.
