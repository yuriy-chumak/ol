...
#### Специальные формы
  ...
  * _case-lambda - эта специальная форма еще не имеет установившегося названия и является оптимизацией для case-lambda из srfi-?.
    * TBD.
  * ol:set - связывание символа в текущем окружении со значением, прототип формы define
    * (ol:set x 12) - связывает символ x со значением 12.
  * ol:let - связывает список переменных со списком значений и выполняет тело
  ...
  * receive - передает несколько значений одновременно в лямбду
  ...
  * ol:if - оператор ветвления, имеет пять аргументов (<тип> <параметр-1> <параметр-2> <если-истина> <если-ложь>) 
   ...
   Прошу заметить, что как бы receive и ol:let не были похожи, это совсем разные операции.
