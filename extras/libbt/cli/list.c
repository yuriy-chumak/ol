...
void
sprintf_items(char *format, struct items* itms)
{
    struct item *p;
    char *it;
	char* ptr = list;
	char* end = list + sizeof(list);
    BTPDQ_FOREACH(p, &itms->hd, entry) {
        if (format) {
            for (it = format; *it; ++it) {
                switch (*it) {
                    case '%':
                        ++it;
                        switch (*it) {
                            case '%': ptr += snprintf(ptr, end-ptr, "%c", '%');                 break;
                            case '#': ptr += snprintf(ptr, end-ptr, "%u",   p->num);            break;
                            case '^': ptr += snprintf(ptr, end-ptr, "%lld", p->rate_up);        break;

                            case 'A': ptr += snprintf(ptr, end-ptr, "%u",   p->pieces_seen);    break;
                            case 'D': ptr += snprintf(ptr, end-ptr, "%lld", p->downloaded);     break;
                            case 'H': ptr += snprintf(ptr, end-ptr, "%u",   p->pieces_have);    break;
                            case 'P': ptr += snprintf(ptr, end-ptr, "%u",   p->peers);          break;
                            case 'S': ptr += snprintf(ptr, end-ptr, "%lld", p->csize);          break;
                            case 'U': ptr += snprintf(ptr, end-ptr, "%lld", p->uploaded);       break;
                            case 'T': ptr += snprintf(ptr, end-ptr, "%u",   p->torrent_pieces); break;

                            case 'd': ptr += snprintf(ptr, end-ptr, "%s",   p->dir);            break;
                            case 'g': ptr += snprintf(ptr, end-ptr, "%lld", p->cgot);           break;
                            case 'h': ptr += snprintf(ptr, end-ptr, "%s",   p->hash);           break;
                            case 'l': ptr += snprintf(ptr, end-ptr, "%s",   p->label);          break;
                            case 'n': ptr += snprintf(ptr, end-ptr, "%s",   p->name);           break;
                            // case 'p': print_percent(p->cgot, p->csize);  break;
                            // case 'r': print_ratio(p->totup, p->csize);   break;
                            // case 's': print_size(p->csize);              break;
                            case 't': ptr += snprintf(ptr, end-ptr, "%c",   p->st);             break;
                            case 'u': ptr += snprintf(ptr, end-ptr, "%lld", p->totup);          break;
                            case 'v': ptr += snprintf(ptr, end-ptr, "%lld", p->rate_down);      break;
                    ...
                    //     }
                    //     break;
                    default: ptr += snprintf(ptr, end-ptr, "%c", *it); break;
                }
            }
        } else {
            list[0] = '\0';
