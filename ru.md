...
layout: page
title:  Summary
date: 2016-06-04 13:25:00 UTC
categories: index
---

   Otus Lisp (произносится как [`отэс лисп]), или ol ([ол]) - чисто функциональный [*](#pure) диалект языка Lisp.
Ol реализован как расширенное подмножество Scheme [R<sup>5</sup>RS](http://www.schemers.org/Documents/Standards/R5RS/), включая, но не ограничиваясь, некоторыми из [SRFI](http://srfi.schemers.org/). Маленький, встраиваемый, кроссплатформенный (Linux, Windows, разные BSD, Android; x86, arm).
  ...
  * маленькая и быстрая [виртуальная машина](?ru/internals/olvm) (всего 42 КБ в бинарном виде [*](#42kb))
  * высокоэффективный, благодаря чистой функциональности языка, [сборщик мусора](?ru/internals/memory)
  * [встраиваемость](?ru/embed),
  * кроссплатформенность (Linux, Windows, Android, FreeBSD, NetBSD, OpenBSD, [MacOS, webOS, etc. - в реальных ближайших планах])
  * поддержка различных архитектур (x86, amd64, arm)
  * возможность компилировать ol-скрипты в бинарный формат, а так же выполнять их
  * динамическое связывание с функциями из "родных" [системных библиотек](?ru/pinvoke) (без использования других языков программирования, таких как C)
  ...
  * и, что главное, это настоящий [Lisp](https://ru.wikipedia.org/wiki/%D0%9B%D0%B8%D1%81%D0%BF)!
   ...
   *** Временно удаленный терминал не работает. Сервер переехал на новое оборудование, на котором выключен режим песочницы. После пересборки деплоя ядра терминал снова будет работать. *** 
...
### Скачать (Download)
   Бинарные сборки на данный момент недоступны, но ведется работа по предоставлению готовых инсталляционных пакетов для Windows, Ubuntu Linux. Исходный код можно забрать на [странице проекта](https://github.com/yuriy-chumak/OL).
  ...
  * "./vm repl" - запустить интерактивную сессию REPL ("repl" - бинарный файл, скомпилированный язык Otus Lisp, ледит в корневой папке проекта), ждать ввода программы с консоли (клавиатуры)
  * "./ol" - аналог "./vm repl"
