...
- [x] webOS: 2.0.
- [x] maxOS: Majove (должны поддерживаться все, включая последнюю).
...
* кроссплатформенность (Linux, Windows, BSD, Android, webOS, Web, macOS, etc.)
...
* 2020
  * обновлена командная строка - теперь можно передавать аргументы в ol-скрипт (обрабатывать через \*vm-args\*)
  * добавлен новый синтаксис для словарей - {}, '{}, `{}. старый синтаксис не удаяется и не объявляется устаревшим
  * добавлен новый синтаксис для векторов (раньше назывались кортежами) - [], '[], `[]. старый синтаксис не удаяется и не объявляется устаревшим
  * добавлены новые математические константы - +inf.0, -inf.0, +nan.0. теперь деление на 0 возвращает соответствующее число (+inf.0, например), а не ошибку
