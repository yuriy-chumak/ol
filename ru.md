...
Ol реализован как расширенное подмножество Scheme R<sup>5</sup>RS (на данный момент заканчивается миграция Ol на [R<sup>7</sup>RS](http://www.r7rs.org/)), включая, но не ограничиваясь, некоторыми из [SRFI](http://srfi.schemers.org/). Маленький, встраиваемый и кроссплатформенный; может работать в песочнице[\*](#sandbox); предоставляет портабельный, высокоуровневый путь для вызова написанного на других языках кода. Вы можете использовать его под GNU/Linux, Windows, Unix, Android и многих других операционных системах на разных (например, x86, arm, aarch64, mips, ppc) архитектурах.
...
- [x] arm: arm920t, snapdragon 801.
  ...
  - идет подготовка к тестированию pmax (на примере DECstation 5000/200), arc, hpcmips, cobalt, evbmips, algor, sgimips, cats, evbarm, netwinder, prep, macppc, pmppc, landisk, dreamcast.
...
### О проекте
Otus Lisp распространяется по лицензии [GPL v3](https://github.com/yuriy-chumak/ol/blob/master/COPYING), так же применима [LGPL v3](https://github.com/yuriy-chumak/ol/blob/master/COPYING.LESSER).
...
* динамическое связывание с функциями из "родных" [системных библиотек](?ru/ffi) (без использования других языков программирования, таких как C), а так же [обратные вызовы](?ru/pinned) из них же
